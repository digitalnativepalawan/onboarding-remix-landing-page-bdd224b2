        ...
        <div className="max-w-xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="glass-card p-4 sm:p-5 mb-6">
            <div className="space-y-3">
              <div>
                <label htmlFor="author" className="block text-xs text-muted-foreground/70 mb-1.5">{t("feedback.nameLabel")}</label>
                <input id="author" type="text" value={authorName} onChange={(e) => setAuthorName(e.target.value)} placeholder={t("feedback.namePlaceholder")} className="w-full px-3 py-2 rounded-lg bg-muted/50 border border-border/30 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all" />
              </div>
              <div>
                <label htmlFor="feedback" className="block text-xs text-muted-foreground/70 mb-1.5">{t("feedback.feedbackLabel")}</label>
                <textarea id="feedback" value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder={t("feedback.messagePlaceholder")} className="w-full h-24 px-3 py-2 rounded-lg bg-muted/50 border border-border/30 text-sm text-foreground placeholder:text-muted-foreground/50 resize-none focus:outline-none focus:ring-1 focus:ring-primary/50 transition-all" />
              </div>
            </div>
          ...
          <div>
            <h3 className="text-xs text-muted-foreground/60 mb-3 flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5" />{t("feedback.recentTitle")} ({feedbackList.length})</h3>
            {isLoading ? (
              ...
              <div className="glass-card p-6 text-center"><p className="text-xs text-muted-foreground/50">{t("feedback.noFeedback")}</p></div>
            ) : (
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span className="text-xs text-foreground/80">{item.author_name || t("feedback.anonymous")}</span>
                          <span className="text-[0.65rem] text-muted-foreground/50 flex items-center gap-1"><Clock className="w-3 h-3" />{format(new Date(item.created_at), 'MMM d')}</span>
                        </div>
                        <p className="text-xs text-muted-foreground/70 leading-relaxed">{item.message}</p>
                      </div>
                    </div>
                  </div>
                ))}
